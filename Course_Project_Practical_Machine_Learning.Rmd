...
### Do Cross Validation and get In-Sample and Out-Of-Sample Error Estimates

I have used cross-validation with the 70% portion of the training dataset in order to pick the predictors and build the model.  Since that data was used to create the model, I must then use the model on the 30% portion that had been set aside in order to calculate the out-of-sample error estimate.  Here we show the actual in-sample error rate, and then the out-of-sample error rate against the 30% reserved portion of the training dataset.

```{r doInSampleErrorRateCalculation}
# Predict using in-sample data to get in-sample error rate (on the 70% portion of the training dataset)
pred.cvtrain <- predict(cvFit, newdata=training)
(sum(pred.cvtrain != training$classe)) / nrow(training) #Show in-sample error rate on training data
```

I then look at the out-of-sample error rate by cross-validating against the independent data (the 30% portion of the training dataset.) 
...
(sum(pred.cvtest != testing$classe)) / nrow(testing)  # Show expected out-of-sample error rate
